...
MAIN_EXAMPLES = ("debug_default_uart.X", "debug_display_i2c.X", "debug_pins_gpio.X")
ALL_EXAMPLES = (
...
class DebugExamplesTests(unittest.TestCase):
    ...
    def test_backend_defines_are_selected(self) -> None:
        text = read_text("debug_default_uart.X", "project_config.h")
        self.assertNotIn("DRV_DEBUG_BACKEND_UART 0", text)

        display = read_text("debug_display_i2c.X", "project_config.h")
        ...
        pins = read_text("debug_pins_gpio.X", "project_config.h")
    ...
    def test_uart_example_has_no_display_or_pins_sources(self) -> None:
        sources = config_sources("debug_default_uart.X")
    ...
    def test_display_i2c_example_has_no_uart_or_parallel_sources(self) -> None:
        sources = config_sources("debug_display_i2c.X")
    ...
    def test_pins_example_has_no_uart_i2c_or_lcd_sources(self) -> None:
        sources = config_sources("debug_pins_gpio.X")
    ...
    def test_disabled_example_has_no_debug_or_backend_sources(self) -> None:
        sources = config_sources("debug_disabled.X")
        self.assertEqual(
            sources,
    ...
    def test_all_main_readmes_cover_required_topics(self) -> None:
        self.assertIn("9600", read_text("debug_default_uart.X", "README.md"))
        self.assertIn("RC6", read_text("debug_default_uart.X", "README.md"))

        display = read_text("debug_display_i2c.X", "README.md")
        self.assertIn("0x27", display.lower())
        self.assertIn("SCL", display)
        self.assertIn("SDA", display)

        pins = read_text("debug_pins_gpio.X", "README.md")
