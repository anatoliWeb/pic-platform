/*
 * File: libraries/input/button/button.c
 */
...
{
    if (btn->use_external_state != 0u)
    {
        return btn->raw_state;
    }

    if (btn->port == (volatile uint8_t*)0)
    {
        return 1u;
    }
...
static void button_init_state(button_t* btn, uint8_t raw)
{
    btn->state = raw;
    btn->last_state = raw;
    btn->raw_state = raw;
                 ...
                 uint8_t pin)
{
    ...
    btn->tris = tris;
    btn->pin = pin;
    btn->use_external_state = 0u;

    if (btn->tris != (volatile uint8_t*)0)
    {
        SET_BIT((*btn->tris), btn->pin);
    }

    raw = button_read_raw(btn);
    button_init_state(btn, raw);
}
...
void button_init_external(button_t* btn, uint8_t initial_raw_state)
{
    ...
    btn->port = (volatile uint8_t*)0;
    btn->tris = (volatile uint8_t*)0;
    btn->pin = 0u;
    btn->use_external_state = 1u;

    button_init_state(btn, (uint8_t)(initial_raw_state != 0u ? 1u : 0u));
}
...
void button_set_raw_state(button_t* btn, uint8_t raw_state)
{
    ...
    if ((btn->use_external_state == 0u) &&
        (btn->port != (volatile uint8_t*)0))
    {
        /* Ignore raw-state injection for regular GPIO-backed buttons. */
        return;
    }

    btn->use_external_state = 1u;
    btn->raw_state = (uint8_t)(raw_state != 0u ? 1u : 0u);
}
