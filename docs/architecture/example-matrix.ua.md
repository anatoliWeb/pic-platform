...
| `examples-projects/xc8/input/digital_input.X` | Simulation/debug demo для debounce raw input і latch behavior | `libraries/input/digital_input/digital_input.c` | немає | Немає | Немає board pins; synthetic raw levels і timestamps | немає | У репозиторії немає | simulation/debug demo |
| `examples-projects/xc8/sensors/tachometer.X` | Simulation/debug demo для pulse-to-RPM conversion і diagnostics | `libraries/sensors/tachometer/tachometer.c` | немає | Немає | Немає board pins; synthetic pulses і timestamps | немає | У репозиторії немає | simulation/debug demo |
| `examples-projects/xc8/actuator/cooldown_output.X` | Simulation/debug demo для requested/active cooldown output behavior | `libraries/actuator/cooldown_output/cooldown_output.c` | немає | Немає | Немає board pins; synthetic requests і timestamps | немає | У репозиторії немає | simulation/debug demo |
...
- Додайте `libraries/actuator/position_drive/position_drive.c`.
- Додайте `drivers/analog/adc/adc.c`, `drivers/gpio/gpio.c`, `drivers/timers/tick/tick.c`, `drivers/timers/timer1/timer1.c`.
- Підключіть `read_raw` callback до `adc_read`, `get_tick` до `tick_get`, а `motor` до пінів H-моста.
- Викликайте `position_drive_process()` регулярно у головному циклі.
