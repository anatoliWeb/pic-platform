...
Бібліотеки не повинні емiтувати `#pragma config`.
...
- `_XTAL_FREQ` - значення частоти, яке використовують delay- та timing-функції.
- `PIC_PLATFORM_WDT_ENABLED`, `PIC_PLATFORM_LVP_ENABLED`,
...
Використовуйте literal CONFIG values у `config_bits.c`:
...
3. `config_bits.c` емiтує реальні рядки `#pragma config` з literal values.
...
- Макроси `#pragma config` для XC8 виявилися ненадійними для PIC18F452.
- Для XC8 і C18 слід використовувати literal CONFIG values у `config_bits.c`.
- `project_config.h` лишається для частоти, таймінгів і допоміжних прапорців.

## Зв'язок із `TODO.md`

Цей шар вважається реалізованим, коли присутні шаблони, checker,
документація та інтеграція з example-проєктами.
